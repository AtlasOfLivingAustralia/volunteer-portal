                        ...
                        <td style="text-align: center;">
                            <span>
                                <g:if test="${taskInstance.fullyTranscribedBy}">
                                    <button class="btn btn-small" onclick="location.href = '${createLink(controller:'task', action:'show', id:taskInstance.id)}'">View</button>
                                    <cl:ifValidator project="${taskInstance.project}">
                                        <g:if test="${taskInstance.status?.equalsIgnoreCase('validated')}">
                                            <button class="btn btn-small" onclick="location.href = '${createLink(controller:'validate', action:'task', id:taskInstance.id)}'">Review</button>
                                        </g:if>
                                        <g:else>
                                            <button class="btn btn-small" onclick="location.href = '${createLink(controller:'validate', action:'task', id:taskInstance.id)}'">Validate</button>
                                        </g:else>
                                    </cl:ifValidator>
                                </g:if>
                                <g:else>
