                ...
                <tr>
                    <td>
                        <div class="row-fluid sort-btn-bar">
                            ...
                            <g:set var="columns" value="${[ ['name', 'Name', 'span2'], ['completed', 'Tasks completed','span3'], ['volunteers', 'Volunteers', 'span2 volunteer-count'], ['institution', 'Sponsoring Institution', 'span3'], ['type', 'Type', 'span2 project-type']]}" />
                            <g:each in="${columns}" var="colkvp">
                                <div class="${colkvp[2]} column-sort-btn"><a href="?sort=${colkvp[0]}&order=${params.sort == colkvp[0] && params.order != 'desc' ? 'desc' : 'asc'}&offset=0&q=${params.q}&${additionalParams}" class="btn ${params.sort == colkvp[0] ? 'active' : ''}">${colkvp[1]}</a></div>
                            </g:each>
                        </div>
                    </td>
                </tr>
                                    ...
                                    <cl:ifAdmin>
                                        ...
                                        <div class="btn-group pull-right ">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <g:link controller="project" action="edit" id="${projectSummary.project.id}"><i class="icon-cog icon-white"></i>&nbsp;Expedition settings</g:link>
                                                </li>
                                                <li class="divider"></li>
                                                <li>
                                                    <g:link controller="task" action="projectAdmin" id="${projectSummary.project.id}"><i class="icon-wrench icon-white"></i>&nbsp;Expedition administration</g:link>
                                                </li>
                                            </ul>
                                        </div>
