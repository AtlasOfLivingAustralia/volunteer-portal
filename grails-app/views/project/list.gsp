...
<cl:headerContent title="${message(code: 'default.projectlist.label', default: "Volunteer for a virtual expedition")}"
...
<div id="content">
    <div class="row">
        ...
        <div class="span6">
            ...
            <g:set var="urlParams"
                   value="${[sort: params.sort ?: "", order: params.order ?: "", offset: 0, q: params.q ?: "", mode: params.mode ?: "", statusFilter: statusFilterMode, activeFilter: activeFilterMode]}"/>

            <div class="btn-group pull-right">
                <a href="${createLink(action: 'list')}" class="btn btn-small ${params.mode != 'thumbs' ? 'active' : ''}"
                   title="View expedition list">
                    <i class="icon-th-list"></i>
                </a>
                <a href="${createLink(action: 'list', params: [mode: 'thumbs'])}"
            ...
            <div class="btn-group pull-right" style="padding-right: 10px">
                <g:each in="${ProjectStatusFilterType.values()}" var="mode">
                    ...
                    <a href="${href}"
                       class="btn btn-small ${statusFilterMode == mode?.toString() ? "active" : ""}">${mode.description}</a>
                </g:each>
            </div>
            ...
            <cl:ifAdmin>
                <div class="btn-group pull-right" style="padding-right: 10px">
                    <g:each in="${ProjectActiveFilterType.values()}" var="mode">
                        ...
                        <a href="${href}"
                           class="btn btn-warning btn-small ${activeFilterMode == mode?.toString() ? "active" : ""}">${mode.description}</a>
                    </g:each>
                </div>
            </cl:ifAdmin>
    ...
    <g:set var="model"
           value="${[extraParams: [statusFilter: statusFilterMode?.toString(), activeFilter: activeFilterMode?.toString()]]}"/>
