...
<div>
    <span><strong class="transcribed-number">${projectSummary.transcribedCount}</strong> of <strong>${projectSummary.taskCount}</strong> tasks transcribed (<strong class="transcribed-percent">${projectSummary.percentTranscribed}%</strong>)</span>
</div>
...
<g:if test="${projectSummary.percentValidated > 0}">
    <div>
        <span><strong class="validated-number">${projectSummary.validatedCount}</strong> task${projectSummary.validatedCount != 1 ? 's': ''} validated (<strong class="validated-percent">${projectSummary.percentValidated}%</strong>)</span>
    </div>
</g:if>
