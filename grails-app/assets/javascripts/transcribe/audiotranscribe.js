...
function wildlifespotter(wsParams, imagePrefix, recordValues, placeholders) {
    jQuery(function ($) {
        ...
        var AUDIO_DETAIL = 'audio-sample-detail';
        var AUDIO_LIST = 'audio-sample-list';
        ...
        $('#ct-container').on('click', '.audio-badge', function () {
            registerSelectionClick($(this));
        });
        ...
        function syncSelections() {
            ...
            var dataItemIndexes = usKeys.map(function (v, i, l) {
                return "[data-item-index='" + v + "']"
            });
            var wsSelectionIndicator = dataItemIndexes.map(function (v, i, l) {
                return v + " .ws-selected";
            });
            var wsSelectorIndicator = dataItemIndexes.map(function (v, i, l) {
                return v + " .ws-selector";
            });
        ...
        function showDetail(index) {
            ...
            addSampleWaveforms(AUDIO_DETAIL);
        }
        ...
        function hideDetail() {
            ...
            destoryDetailSurfers(AUDIO_DETAIL);
        }
        ...
        $('#ct-container').on('click', '.audio-info', function (e) {
            e.stopPropagation();
            var $this = $(this);
            // var idxElem = $this.closest('[data-item-index]');
            // var idx = idxElem.data('item-index');
        ...
        var wavesurfers = [];
        var detailSurfers = [];
        function addSampleWaveforms(selector) {
            //console.log('adding sample waves3');
            var surferList = [].map.call(document.querySelectorAll("div." + selector), function (element) {
            ...
            surferList.forEach(function (item, index) {
                item.wave.load(item.url);
                var button = document.querySelector('.' + selector + '-play[data-action-play="' + item.playLink + '"]');
                button.addEventListener('click', item.wave.playPause.bind(item.wave));
                button.addEventListener('click', function(e) {
                    var $this = $(this);
                    togglePlayButton($this);
                });
                item.wave.on('finish', function () {
                    console.log('audio finished');
                    togglePlayButton(button, true);
                });
                item.button = button;

                addSurferToList(item, selector);
            });
        }

        function addSurferToList(item, selector) {
            var exists = false;
            wavesurfers.forEach(function (existingItem, index) {
                if (item.selector === existingItem.selector && item.playLink === existingItem.playLink) {
                    exists = true;
                }
            });
            if (!exists) wavesurfers.push(item);
        }

        function destoryDetailSurfers(selector) {
        ...
        function stopAudio() {
            wavesurfers.forEach(function (item, index) {
                console.log("Wavefile: index: [" + index + "], selector: [" + item.selector + "], link: [" + item.playLink + "]");
                console.log(item.wave.isPlaying());
                // console.log(item);
                if (item.wave.isPlaying()) {
                    item.wave.stop();
                    togglePlayButton(item.button, true);
                }
            });

            // console.log("stopping inline player");
            // console.log(window.inlinePlayer);
            if (window.inlinePlayer !== undefined && window.inlinePlayer !== null) {
                window.inlinePlayer.stopAll();
            }
        }

        // Cycling Thumbnails
        function cycleImages() {
            $('#ct-animals-list .cycler').filter(function (i) {
        ...
        function generateFormFields() {
            ...
            _.each(selectedIndicies, function (value, key, list) {
                mu.appendTemplate($ctFields, 'input-template', {
                    id: 'recordValues.' + i + '.vernacularName',
                    value: wsParams.animals[key].vernacularName
                });
                mu.appendTemplate($ctFields, 'input-template', {
