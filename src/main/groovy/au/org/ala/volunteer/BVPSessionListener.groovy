...
class BVPSessionListener implements HttpSessionListener {
    ...
    void sessionCreated(HttpSessionEvent e) {
        logger.debug "Session created: ${e.session.id}${assertionDetails(e.session)}"
    }

    void sessionDestroyed(HttpSessionEvent e) {
        use (TimeCategory) {
            TimeDuration lastAccessedDuration = new Date() - new Date(e.session.lastAccessedTime)
            logger.debug "Session destroyed: ${e.session.id}${assertionDetails(e.session)}, last accessed: ${new Date(e.session.lastAccessedTime)} - ${lastAccessedDuration} ago"
        }
