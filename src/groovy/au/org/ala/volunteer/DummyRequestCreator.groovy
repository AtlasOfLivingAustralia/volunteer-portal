...
class DummyRequestCreator { //implements HttpServletRequest {
    ...
    static HttpServletRequest createInstance(String requestURI) {
        return Proxy.newProxyInstance( HttpServletRequest.class.getClassLoader(), [HttpServletRequest.class] as Class[], new DummyRequestInvocationHandler(requestURI)) as HttpServletRequest;
    }
}

class DummyRequestInvocationHandler implements InvocationHandler {
    ...
    public DummyRequestInvocationHandler(String requestURI) {
    ...
    Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        switch (method.name) {
            ...
            case "getRemotePort":
                def methodName = method.name.replaceAll( /([A-Z])/, / $1/ ).toLowerCase().substring(4)
                throw new UnsupportedOperationException("You cannot read the ${methodName} in non-request rendering operations")
            case "getLocale":
